...
def analyze_request(user_request: str, llm_client: LLMClient) -> Tuple[Optional[APICall], Optional[LogQuery], str, Optional[str]]:
    ...
    if is_dangerous:
        return None, None, f"درخواست خطرناک: {matched}", None
    ...
    if not llm_response:
        return None, None, "خطا در LLM", None
    ...
    if not api_call and not log_query:
        return None, None, "پاسخ نامعتبر", None
    
    if api_call and not validate_operation(api_call.operation):
        api_call.safe = False
        api_call.warnings = [f"عملیات '{api_call.operation}' مجاز نیست"]
...
def display_api_call(api_call: APICall):
    print("\n" + "="*60)
    print("📋 پیشنهاد API Call")
    print("="*60)
    print(f"عملیات: {api_call.operation}")
    print(f"توضیحات: {api_call.description}")
    print(f"روش: {api_call.method}")
...
def display_log_query(log_query: LogQuery, extra: Optional[str] = None):
    ...
    print("📊 پرس‌وجوی لاگ")
    print("="*60)
    print(f"دسته: {log_query.category}")
    if log_query.subcategory:
        print(f"زیردسته: {log_query.subcategory}")
    print(f"تعداد: {log_query.limit}")
    if extra:
        print(f"نوع: {extra}")
    print("="*60)
...
def display_log_results(result: Dict):
    ...
    if "search_keyword" in result:
        print(f"جستجو: {result['search_keyword']}")
        print(f"یافت شده: {result['result_count']}")
        ...
        print(f"تعداد: {analysis.get('total_logs', 0)}")
        ...
        if atype == "summary":
            ...
            print(f"\n🚫 مسدود: {analysis.get('total_blocked', 0)}")
            print(f"درصد: {analysis.get('blocked_percentage', 0):.2f}%")
        
        elif atype == "security_events":
            print(f"\n🔒 رویدادها: {analysis.get('total_security_events', 0)}")
...
def yes_no(prompt: str, default: bool = False) -> bool:
    ...
    return response in ('y', 'yes', 'بله') if response else default
...
def interactive_session(api: FortigateAPIHelper, llm_client: LLMClient, log_analyzer: FortiGateLogAnalyzer, args):
    print("\n" + "="*60)
    print("   🤖 دستیار FortiGate (با تحلیل لاگ)")
    print("="*60)
    print("مثال‌ها:")
    print("  • برای 10.10.10.10 آدرس بساز")
    print("  • لاگ های ترافیک یک ساعت گذشته")
    print("  • بیشترین IP های منبع")
    print("  • جستجو برای 192.168.1.50")
    print("\n'exit' برای خروج")
    print("="*60 + "\n")
    ...
    while True:
        try:
            ...
            if not req or req.lower() in ['exit', 'quit', 'خروج']:
                print("\n👋 خداحافظ!")
                break
            ...
            if api_call:
                ...
                if not api_call.safe:
                    print("\n⚠️ ناامن!")
                    if not args.force and not yes_no("ادامه؟"):
                        save_audit(action)
                        continue
