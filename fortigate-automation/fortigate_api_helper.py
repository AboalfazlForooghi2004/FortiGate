...
logger = setup_syslog_logger("fortigate_api_helper_v2")
...
class FortigateAPIHelper:
    ...
    def __init__(
        ...
        self.base_url = base_url.rstrip("/").replace("/api/v2/cmdb", "/api/v2").replace("/api/v2/cmdb/", "/api/v2")
        self.vdom = vdom
        ...
        logger.info(
            "FortigateAPIHelper v2 initialized (vdom=%s, base_url=%s, ssl_verify=%s)",
    ...
    def monitor_get(self, endpoint: str, params: Optional[Dict] = None) -> Any:
        """GET request for monitor endpoints"""
