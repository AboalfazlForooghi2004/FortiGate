...
# ==================== VIP Operations ====================
@handle_errors(error_handler, {"phase": "phase5", "operation": "vip_check"})
def vip_exists(api: FortigateAPIHelper, vip_name: str) -> Tuple[bool, Optional[Dict]]:
    """
    Check if VIP exists
    Returns (exists, vip_config)
    """
    try:
        ...
        if "not found" in str(e).lower() or e.details.get("status") == 404:
            return False, None
        raise


@handle_errors(error_handler, {"phase": "phase5", "operation": "find_references"})
def find_all_vip_references(api: FortigateAPIHelper, vip_name: str) -> List[Dict]:
    """
    Find all references to the VIP in:
    - VIP groups
...
def remove_vip_from_group(api: FortigateAPIHelper, group_name: str, 
    ...
    try:
        ...
        # Remove VIP from members
        members = [safe_get_name(m) for m in grp.get("member", []) 
                  if safe_get_name(m) != vip_name]
        
        if not members:
            # If no members left, we should probably delete the group
...
def process_vip_deletion(api: FortigateAPIHelper, vip_name: str,
    ...
    if refs:
        ...
        # Ask for confirmation to remove references
